        ...
        <div class="right_list" v-for="(item,index) in datas.data" :class="{none:index !=nav_eq}">
          <div class="list" v-for="x in item.level" :class="{none:x.num !=level}">
            ...
            <div class="pagination">
              <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage" layout="total,prev,pager,next,jumper" :page-size="10" background :total="pageNum">
              </el-pagination>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.$http.get('http://www.nxqybank.com/pc/index.php/Index/bianming').then(response => {
